...
def test_hierarchical_data_structure():
    ...
    try:
        ...
        if result.get('success'):
            print("✅ Hierarchical data preparation successful!")
            print(f"✅ Total rows: {len(result['data'])}")
            print(f"✅ Columns: {result['columns']}")
            print(f"✅ Grid points count: {result['grid_points_count']}")
            ...
            if len(data) > 0:
                ...
                print(f"✅ Parent has children: {parent_row.get('children_count', 0)}")

                if len(data) > 1:
                    child_rows = data[1:]
                    print(f"✅ Number of child rows: {len(child_rows)}")

                    # Check first child row structure
                    first_child = child_rows[0]
                    print(f"✅ First child: {first_child['name']}")
                    print(f"✅ Child hazards: {list(first_child['hazards'].keys())}")
                    ...
                    for hazard in selected_hazards:
                        if hazard in first_child['hazards']:
                            ...
                            print(f"✅ {hazard} data - Value: {hazard_data.get('value')}, "
            ...
            print(f"❌ Hierarchical data preparation failed: {result.get('error')}")
            return False
...
def test_data_structure_consistency():
    ...
    try:
        ...
        if not asset:
            print("⚠️  No assets found for consistency test")
            return True
        ...
        if not hazards:
            print("⚠️  No completed analysis results found")
            return True

        result = prepare_hierarchical_exposure_data(asset, hazards)

        if not result.get('success'):
            print(f"❌ Failed to prepare data: {result.get('error')}")
            return False
        ...
        for field in required_parent_fields:
            if field not in parent_row:
                print(f"❌ Missing required field in parent row: {field}")
                return False
        ...
        for child in data[1:]:
            ...
            for field in required_child_fields:
                if field not in child:
                    print(f"❌ Missing required field in child row: {field}")
                    return False

            # Verify parent reference
            if child['parent_id'] != parent_row['id']:
                print(f"❌ Child parent_id mismatch: {child['parent_id']} != {parent_row['id']}")
                return False
        ...
        for row in data:
            for hazard in hazards:
                if hazard not in row['hazards']:
                    print(f"❌ Missing hazard data for {hazard} in row {row['name']}")
                    return False
